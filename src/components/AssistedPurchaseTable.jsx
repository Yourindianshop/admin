...
const AssistedPurchaseTable = ({iswhiteLabeling}) => {
  ...
  const getData = async ()=>{
    const wid = mg?.Wid;
    const dt = await fetchreq("GET",iswhiteLabeling?"getWhiteLabelingRequest":"getAPRManager",{});
      ...
      <table className="assisted-purchase-table">
        <thead>
          <tr>
            <th>Request ID</th>
            <th>Customer ID</th>
            {iswhiteLabeling && <th>Business Name</th>}
            {iswhiteLabeling && <th>Business Phone</th>}
            <th>Product Name</th>
            <th>Product Brand</th>
            <th>Product Description</th>
            <th>Other Info</th>
            {iswhiteLabeling&& <th>Country</th>}
            <th>Product Image</th>
        ...
        <tbody>
          {APR && APR.map((request, index) => (
            <tr key={index} className="asr-row">
              <td>{request.Aid}</td>
              <td>{request.email}</td>
              {iswhiteLabeling && <td>{request.name}</td>}
              {iswhiteLabeling && <td>{request.phone}</td>}
              <td>{request.Name}</td>
              <td>{request.Brand}</td>
              <td>
              ...
              <td>{request.Other}</td>
              {iswhiteLabeling && <td>{request.Country}</td>}
              <td>
                <img
